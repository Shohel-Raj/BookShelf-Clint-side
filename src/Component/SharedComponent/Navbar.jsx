...
console.log(userRole)
    const handleBTN = () => navigate('/');

    const handleLogout = () => {
        signOutUser()
            .then(() => toast.warn('Logged out. Login for a better experience.'))
            .catch((error) => toast.error('Something went wrong: ' + error.code));
    };
    ...
    useEffect(() => {
        const fetchRole = async () => {
            ...
            try {
                ...
                const data = await getUserRole(user?.email, user?.accessToken);
                setUserRole(data?.role || "");
            } catch (err) {
                toast.error("Something went wrong while fetching user role");
            } finally {
    ...
    if (userRoleLoading) return <Loader />;
                ...
                <li>
                        <NavLink to="/adminDashBoard" className={({ isActive }) =>
                            isActive
                                ? 'font-bold border-2 uppercase dark:text-green-400'
                                : 'uppercase hover:text-green-500 dark:hover:text-green-400'
                        } onClick={() => setIsMenuOpen(false)}>Dashboard</NavLink>
                    </li>
                </> )
            }
        </>
        ...
        <div className="navbar px-0 w-11/12 md:w-10/12 mx-auto duration-300">
            ...
            <div className="hidden md:flex navbar-end gap-2">
                {user ? (
                    ...
                    <Link to="/loginSignInPage" className="btn bg-green-500 text-white hover:bg-green-400 hover:text-black transition-colors duration-200">
            ...
            {isMenuOpen && (
                <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 md:hidden">
                    <div className="p-4 flex justify-between items-center border-b border-gray-300 dark:border-gray-700 shadow-sm">
                        ...
                        <button className="btn btn-sm btn-ghost text-2xl" onClick={() => setIsMenuOpen(false)}>✕</button>
                    </div>
                    <ul className="menu p-4 space-y-2">{links}</ul>
                    <div className="p-4">
                        {user ? (
                            <button onClick={() => { handleLogout(); setIsMenuOpen(false); }} className="btn w-full bg-green-500 text-white hover:bg-green-400 hover:text-black transition-colors duration-200">
                                Logout
                            </button>
                        ) : (
                            <Link to="/loginSignInPage" className="btn w-full bg-green-500 text-white hover:bg-green-400 hover:text-black transition-colors duration-200" onClick={() => setIsMenuOpen(false)}>
                                Login
                            </Link>
                        )}
                    </div>
